...
export default function EditJobContent() {
  ...
  const [attachment, setAttachment] = useState(null);       // new file chosen by user
  const [existingFileUrl, setExistingFileUrl] = useState(null); // existing URL from backend
  const [jobStatus, setJobStatus] = useState("");
  

  const canEdit = jobStatus === "ACTIVE";
  ...
  useEffect(() => {
    const fetchJob = async () => {
      try {
        const response = await apiClient.get(`/api/dashboard/get-post/${id}`);
        ...
        setJobData({
          ...
          projectStartTime: job.projectStartTime || null,
          projectEndTime: job.projectEndTime || null,
        });

        setSelectedSkills(job.requiredSkills || []);
        setExistingFileUrl(job.file || null);
  ...
  const removeNewAttachment = () => {
    ...
    toast.info("New file removed. Existing file will be kept.");
  };
  ...
  const handleUpdateJob = async () => {
    ...
    try {
      ...
      const jobDto = {
        ...
        // required by JobDto record but not used by updatePost
        file: null,
        status: null,
        phase: null,
        clientDto: null,
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
            ...
            <CardDescription>
                Update the information for your job post
            </CardDescription>

            {jobStatus !== "ACTIVE" && (
                <div className="flex items-center gap-2 p-3 mt-3 rounded-md bg-yellow-50 border border-yellow-300 text-yellow-700">
                <AlertCircle className="h-4 w-4" />

                <span>
                    This job is not active. Editing is disabled.
                </span>
                </div>
            )}
                    ...
                    {skillSuggestions
                      ...
                      .map((skill) => (
                        <Button
                          ...
                          disabled={!canEdit || selectedSkills.length >= 15}
                        >
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mb-3">
                    Upload relevant documents, requirements, or reference materials
                  </p>
                </div>
                    ...
                    {existingFileUrl && (
                        ...
                        {!attachment && canEdit && (
                            ...
                            Uploading a new file will replace this attachment.
                            </p>
                        )}
                ...
                {!attachment ? (
                  <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-6 text-center">
                    ...
                    <p className="text-xs text-muted-foreground mt-2">
                      Supported formats: PDF, DOC, DOCX, TXT, JPG, PNG (Max 500KB)
                    </p>
                  </div>
                ) : (
                  <div className="space-y-2">
                    <h4 className="text-sm font-medium">New File Selected</h4>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground whitespace-pre-line">
                      {jobData.jobDescription || "Project description will appear here..."}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground whitespace-pre-line">
                      {jobData.requirement || "Project requirements will appear here..."}
                    </p>
                  </div>
                      ...
                      <div>
                        ...
                        <p className="font-semibold text-sm">
                          {jobData.projectStartTime && jobData.projectEndTime
                            ? `${format(new Date(jobData.projectStartTime), "MMM dd, yyyy")} - ${format(new Date(jobData.projectEndTime), "MMM dd, yyyy")}`
                            : "Not set"}
                        </p>
                      </div>
                    </div>
                  ...
                  {/* Attachment preview in review */}
                  {(existingFileUrl || attachment) && (
                    <div className="pt-4 border-t">
                      ...
                      {attachment ? (
                        ...
                        <a
                          ...
                          className="flex items-center space-x-2 p-2 border rounded bg-muted/20 w-fit hover:bg-muted/40"
                        >
                          <FileText className="h-4 w-4 text-muted-foreground" />
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <ul className="text-sm text-blue-800 space-y-1">
                      ...
                      <li>• Job can only be updated while it is ACTIVE</li>
                    </ul>
                  </div>
                </div>
              </div>
