...
const freelancerBidHistory = [
  ...
  },
  {
    ...
    responseTime: "1 day",
    description: "Optimize our existing e-commerce platform for better performance and SEO...",
  },
]
...
const clientJobHistory = [
  {
    ...
    description: "Looking for an experienced React developer to join our SaaS platform development...",
  },
  {
    id: 2,
    ...
    description: "Need a complete brand identity package including logo, colors, and guidelines...",
  },
  {
    id: 3,
    ...
    description: "Looking for technical content writers to create engaging blog posts...",
  },
]
...
export default function BidHistoryContent({ userRole }) {
  ...
  const data = userRole === "freelancer" ? freelancerBidHistory : clientJobHistory
  ...
  const getStats = () => {
    if (userRole === "freelancer") {
      return {
        ...
        pending: freelancerBidHistory.filter((bid) => bid.status === "Pending").length,
        rejected: freelancerBidHistory.filter((bid) => bid.status === "Rejected").length,
        successRate: Math.round(
          (freelancerBidHistory.filter((bid) => bid.status === "Won").length / freelancerBidHistory.length) * 100,
        ),
      }
    } else {
      return {
        ...
        inReview: clientJobHistory.filter((job) => job.status === "In Review").length,
        completed: clientJobHistory.filter((job) => job.status === "Completed").length,
        totalProposals: clientJobHistory.reduce((sum, job) => sum + job.proposals, 0),
      }
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div>
        <h1 className="text-2xl md:text-3xl font-bold tracking-tight">
          {userRole === "freelancer" ? "Bid History" : "Job Posting History"}
        </h1>
      ...
      <div className="grid gap-4 grid-cols-2 sm:grid-cols-3 lg:grid-cols-5">
        {userRole === "freelancer" ? (
          <>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-xs md:text-sm font-medium">Total Bids</CardTitle>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-xl md:text-2xl font-bold text-green-600">{stats.won}</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Card className="col-span-2 sm:col-span-1">
              ...
              <CardContent>
                <div className="text-xl md:text-2xl font-bold text-blue-600">{stats.successRate}%</div>
              </CardContent>
            </Card>
          </>
        ) : (
          <>
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-xs md:text-sm font-medium">Hired</CardTitle>
                <div className="h-3 w-3 md:h-4 md:w-4 rounded-full bg-green-500" />
              </CardHeader>
              <CardContent>
                <div className="text-xl md:text-2xl font-bold text-green-600">{stats.hired}</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-xs md:text-sm font-medium">In Review</CardTitle>
                <div className="h-3 w-3 md:h-4 md:w-4 rounded-full bg-yellow-500" />
              </CardHeader>
              <CardContent>
                <div className="text-xl md:text-2xl font-bold text-yellow-600">{stats.inReview}</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Card className="col-span-2 sm:col-span-1">
              ...
              <CardContent>
                <div className="text-xl md:text-2xl font-bold">{stats.totalProposals}</div>
              </CardContent>
            </Card>
          </>
        )}
          ...
          <div className="flex flex-col space-y-4 md:flex-row md:space-y-0 md:space-x-4">
            <div className="flex-1">
              <div className="relative">
                ...
                <Input
                  placeholder={`Search by ${userRole === "freelancer" ? "project title or client" : "job title"}...`}
                  value={searchTerm}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg md:text-xl">
            {userRole === "freelancer" ? `Your Bids (${filteredData.length})` : `Your Jobs (${filteredData.length})`}
          </CardTitle>
        ...
        <CardContent>
          <div className="space-y-4">
            {filteredData.map((item) => (
              <div key={item.id} className="rounded-lg border p-4 md:p-6 space-y-4">
                  ...
                  {userRole === "freelancer" ? (
                    <>
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="text-muted-foreground font-semibold">{item.bidAmount}</div>
                      </div>
                      <div className="space-y-1">
                        <div className="font-medium">Submitted</div>
                        <div className="text-muted-foreground">{item.submittedAt}</div>
                      </div>
                    </>
                  ) : (
                    <>
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="text-muted-foreground">{item.proposals}</div>
                      </div>
                      <div className="space-y-1">
                        <div className="font-medium">Hired</div>
                        <div className="text-muted-foreground">{item.hiredFreelancer || "None"}</div>
                      </div>
                      <div className="space-y-1">
                        <div className="font-medium">Posted</div>
                        <div className="text-muted-foreground">{item.postedAt}</div>
                      </div>
                    </>
                  )}
                </div>
