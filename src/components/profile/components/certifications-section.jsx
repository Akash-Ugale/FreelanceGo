      ...
      reader.onloadend = () => {
        if (certId) {
          setItems(
            items.map((c) =>
              c.id === certId
                ? { ...c, certificateImage: reader.result, file }
                : c
            )
          );
        } else {
        ...
        {isEditing ? (
          <div className="space-y-3">
            {certifications.map((cert, idx) => (
              <div
                key={idx}
                className="p-3 border border-border rounded-md space-y-2 bg-secondary/30"
              >
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setItems(
                          items.map((c) =>
                            c.id === cert.id
                              ? { ...c, certificateName: e.target.value }
                              : c
                          )
                        )
                      }
                        ...
                        <label className="cursor-pointer flex-1">
                          ...
                          >
                            <span>
                              ...
                              {cert.certificateImage ? "Change" : "Upload"}{" "}
                              Image
                            </span>
                          </Button>
                          <input
                            ...
                            accept="image/*"
                            onChange={(e) =>
                              handleCertificateImageUpload(e, cert.id)
                            }
                            className="hidden"
                          />
                        </label>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    {newCert.certificateImage && (
                      <Avatar className="h-12 w-12 border border-border">
                        <AvatarImage
                          src={newCert.certificateImage || "/placeholder.svg"}
                        />
                ...
                >
                  ...
                  {cert.certificateUrl && (
                    <Button
                      ...
                      onClick={() => setSelectedCertImage(cert.certificateUrl)}
                    >
